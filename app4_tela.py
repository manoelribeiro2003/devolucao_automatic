...
def pular_linha():
    nav.contador()
    label_info.config(text="Linha: "+str(nav.get_contador()+2))

def alterar_linha():
    nova_linha = int(campo_texto.get())
    if nova_linha == None or nova_linha == 0:
        nova_linha = 1
    nav.alter_contador(nova_linha-2)
...
# Criando uma label
label_info = Label(tab1, text="Linha: "+str(nav.get_contador()))
...
Button(tab1, text='Inserir Código', font=('Arial', 15), command=inserir_codigo).pack(padx=8, pady=8, fill=BOTH)
...
Button(tab1, text='Sair', font=('Arial', 15), command=sair).pack(padx=8, pady=8, fill=BOTH, )
